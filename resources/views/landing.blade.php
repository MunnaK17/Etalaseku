    ...
    <div class="min-h-screen overflow-x-hidden">
        <header class="fixed left-0 right-0 top-0 z-50 px-4 py-3 md:px-8" role="banner">
            <nav class="nav-container mx-auto flex max-w-7xl items-center justify-between rounded-2xl border px-5 py-3 backdrop-blur-xl md:px-8" aria-label="Navigasi utama">
                <a href="{{ route('home') }}" class="flex shrink-0 items-center gap-2" aria-label="EtalaseKu beranda">
                ...
                <div class="hidden items-center gap-3 md:flex">
                    ...
                    @auth
                        ...
                        <div class="relative" x-data="{ open: false }">
                            ...
                            <div x-show="open"
                                ...
                                <div class="py-2">
                                    <a href="{{ route('seller.dashboard') }}" class="flex items-center gap-3 px-4 py-2 text-sm transition-colors" style="color: var(--text-secondary);">
                                    ...
                                    @if($store)
                                        <a href="{{ $store->public_url }}" target="_blank" class="flex items-center gap-3 px-4 py-2 text-sm transition-colors" style="color: var(--text-secondary);">
                                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    ...
                    @else
                        <a href="{{ route('login') }}" class="rounded-lg border px-4 py-2 text-sm font-medium transition" style="border-color: var(--border-color); color: var(--text-secondary);">Masuk</a>
                        <a href="{{ route('register') }}" class="rounded-lg px-5 py-2 text-sm font-semibold transition" style="background: var(--accent); color: #000;">Daftar Gratis</a>
                    @endauth
                </div>
            ...
            <div id="mobileMenu" class="mx-auto mt-2 hidden max-w-7xl rounded-2xl px-5 py-4 backdrop-blur-xl md:hidden" style="background: var(--nav-bg); border: 1px solid var(--border-color);">
                <div class="space-y-2">
                    ...
                    @auth
                        <a href="{{ route('seller.dashboard') }}" class="block rounded-lg border px-4 py-2 text-center text-sm font-medium text-center" style="border-color: var(--border-color); color: var(--text-secondary);">Dashboard</a>
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                    ...
                    @else
                        <a href="{{ route('login') }}" class="block rounded-lg border px-4 py-2 text-center text-sm font-medium" style="border-color: var(--border-color); color: var(--text-secondary);">Masuk</a>
                        <a href="{{ route('register') }}" class="block rounded-lg px-4 py-2 text-center text-sm font-semibold text-center" style="background: var(--accent); color: #000;">Daftar Gratis</a>
                    @endauth
                </div>
            </div>
        </header>
        ...
        <main id="hero">
                    ...
                    <div class="grid items-center gap-10 md:grid-cols-2 md:gap-16">
                        <div class="animate-on-scroll">
                            ...
                            <div class="mt-8 flex flex-col gap-3 sm:flex-row">
                                <a href="{{ route('register') }}" class="btn-glow inline-flex items-center justify-center gap-2 rounded-xl px-7 py-3.5 font-semibold shadow-lg btn-primary-enhanced transition-all duration-300" style="background: var(--accent); color: #000; box-shadow: 0 10px 25px -5px var(--shadow-color, rgba(255, 215, 0, 0.3));">
                                    Mulai Gratis
                                ...
                                <div class="mt-5 space-y-3">
                                    @foreach ([['Kue Ulang Tahun', 'Rp 85.000'], ['Kue Kering Lebaran', 'Rp 45.000'], ['Cupcake Kustom', 'Rp 25.000']] as $product)
                                        <div class="flex items-center gap-3 rounded-xl p-3 transition-all duration-300 hover:scale-[1.02] hover:bg-[var(--bg-tertiary)]" style="background: var(--bg-tertiary);">
                                            ...
                                            <div>
                                                <p class="text-sm font-semibold" style="color: var(--text-primary);">{{ $product[0] }}</p>
                                                <p class="text-xs" style="color: var(--text-muted);">{{ $product[1] }}</p>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 stagger-children">
                        @foreach ($fiturUtama as $index => $item)
                            <article class="feature-card rounded-2xl border p-5 animate-on-scroll" style="background: var(--bg-card); border-color: var(--border-color); --stagger-index: {{ $index }};">
                                ...
                                <p class="mt-2 text-sm leading-relaxed" style="color: var(--text-secondary);">{{ $item['desc'] }}</p>
                            </article>
                        @endforeach
                    </div>
                </div>
                    ...
                    <div class="store-carousel-wrapper">
                        ...
                        <div class="store-carousel" id="storeCarousel">
                            @foreach($showcaseStores as $store)
                                <x-store-preview-card :store="$store" />
                            @endforeach
                        </div>
                ...
                <div class="mx-auto max-w-6xl">
                    ...
                    <div class="grid gap-4 md:grid-cols-3 stagger-children">
                        @foreach ($layanan as $index => $item)
                            <article class="feature-card rounded-2xl border p-6 animate-on-scroll" style="background: var(--bg-card); border-color: var(--border-color); --stagger-index: {{ $index }};">
                                <h3 class="font-semibold" style="color: var(--text-primary);">{{ $item['title'] }}</h3>
                                <p class="mt-2 text-sm leading-relaxed" style="color: var(--text-secondary);">{{ $item['desc'] }}</p>
                            </article>
                        @endforeach
                ...
                <div class="mx-auto max-w-5xl">
                    ...
                    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 stagger-children">
                        @foreach ($audiences as $index => $audience)
                            <div class="audience-badge rounded-2xl border p-5 text-center font-semibold animate-on-scroll" style="background: var(--bg-card); border-color: var(--border-color); color: var(--text-primary); --stagger-index: {{ $index }};">{{ $audience }}</div>
                        @endforeach
                    </div>
                </div>
            </section>
                ...
                <div class="relative z-10 mx-auto max-w-4xl text-center animate-on-scroll">
                    ...
                    <div class="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4 stagger-children" style="max-width: 700px; margin-left: auto; margin-right: auto;">
                        @foreach ([
                            ...
                            <div class="pricing-card rounded-xl border p-4 animate-on-scroll" style="background: var(--bg-card); border-color: var(--border-color); --stagger-index: {{ $index }};">
                                <p class="text-2xl font-bold" style="color: var(--accent);">{{ $stat['value'] }}</p>
                                <p class="text-sm" style="color: var(--text-muted);">{{ $stat['label'] }}</p>
                            </div>
                        @endforeach
                    </div>

                    <a href="{{ route('inclusive-program.form') }}" class="btn-glow btn-primary-enhanced mt-8 inline-flex items-center gap-2 rounded-xl px-8 py-4 font-semibold transition-all duration-300" style="background: linear-gradient(135deg, #a855f7, #ec4899); color: #fff;">
                ...
                <div class="mx-auto max-w-5xl">
                    ...
                    <div class="grid gap-5 md:grid-cols-2 stagger-children">
                        <article class="pricing-card rounded-2xl border p-6 animate-on-scroll" style="background: var(--bg-card); border-color: var(--border-color); --stagger-index: 0;">
                            ...
                            <a href="{{ route('register') }}" class="mt-6 block rounded-xl px-5 py-3 text-center font-semibold transition-all duration-300 hover:scale-[1.02] active:scale-[0.98]" style="background: var(--bg-tertiary); color: var(--text-primary); border: 1px solid var(--border-color);">Daftar Gratis</a>
                        </article>
                        ...
                        <article class="pricing-card popular relative rounded-2xl border-2 p-6 shadow-lg animate-on-scroll" style="background: var(--bg-card); border-color: var(--accent); box-shadow: 0 20px 40px -12px var(--shadow-color, rgba(217, 119, 6, 0.2)); --stagger-index: 1;">
                            ...
                            <a href="{{ route('register') }}?plan=pro" class="btn-glow btn-primary-enhanced mt-6 block rounded-xl px-5 py-3 text-center font-semibold transition-all duration-300" style="background: var(--accent); color: #000;">Upgrade ke Pro</a>
                        </article>
                    </div>
                </div>
            </section>
                ...
                <div class="mx-auto max-w-3xl">
                    ...
                    <div class="space-y-3 stagger-children">
                        @foreach ($faq as $index => $item)
                            <article class="faq-item overflow-hidden rounded-2xl border animate-on-scroll" style="background: var(--bg-card); border-color: var(--border-color); --stagger-index: {{ $index }};">
                                <button type="button" class="faq-button flex w-full items-center justify-between gap-4 px-5 py-4 text-left transition-all duration-300 hover:bg-[var(--bg-tertiary)]" aria-expanded="false">
                                    <span class="text-sm font-medium" style="color: var(--text-primary);">{{ $item['q'] }}</span>
                                    <span class="faq-icon transition-transform duration-300" style="color: var(--text-muted);">⌄</span>
                                </button>
                                <div class="faq-answer px-5 pb-4">
                                    <p class="text-sm leading-relaxed" style="color: var(--text-secondary);">{{ $item['a'] }}</p>
                                </div>
                            </article>
                        @endforeach
                    </div>
            ...
            <section class="relative overflow-hidden px-6 py-16 md:py-24">
                ...
                <div class="relative z-10 mx-auto max-w-3xl text-center animate-on-scroll">
                    ...
                    <a href="{{ route('register') }}" class="btn-glow btn-primary-enhanced mt-8 inline-flex rounded-xl px-8 py-3.5 font-semibold transition-all duration-300 hover:scale-[1.03] active:scale-[0.98]" style="background: var(--accent); color: #000;">Mulai Gratis Sekarang</a>
                </div>
            </section>
        </main>
        ...
        <footer class="border-t px-6 py-10 md:py-14" style="border-color: var(--border-color);">
            <div class="mx-auto flex max-w-6xl flex-col gap-6 md:flex-row md:items-center md:justify-between">
                ...
                <p class="text-xs" style="color: var(--text-muted);">&copy; {{ date('Y') }} EtalaseKu. Semua hak dilindungi.</p>
            </div>
        </footer>
    </div>
