                ...
                <div class="p-5">
                    <div class="flex items-start gap-5">
                        ...
                        <div class="flex-shrink-0">
                            @if($store->logo)
                                <img src="{{ $store->logo }}" alt="{{ $store->name }}" class="h-20 w-20 rounded-full object-cover border-2 border-gray-100">
                            @else
                        ...
                        <!-- Info -->
                        <div class="flex-1">
                            <h2 class="text-xl font-bold text-gray-900">{{ auth()->user()->name ?? 'Seller' }}</h2>
                            <p class="text-sm text-indigo-600 mt-0.5">{{ $store->public_url }}</p>
...
<button onclick="copyLink()" class="inline-flex items-center gap-1.5 px-3 py-1.5 border border-gray-300 text-gray-700 rounded-lg text-xs font-semibold hover:bg-gray-50 transition">
            ...
            <div class="mt-6">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                    <a href="{{ route('seller.products.create') }}" class="flex flex-col items-center justify-center p-4 bg-white border border-gray-200 rounded-xl hover:border-indigo-300 hover:shadow-sm transition group">
                    ...
                    <a href="{{ route('seller.products.create') }}?type=digital" class="flex flex-col items-center justify-center p-4 bg-white border border-gray-200 rounded-xl hover:border-indigo-300 hover:shadow-sm transition group">
                ...
                <div class="p-5">
                    ...
                    <div class="flex items-center gap-6 mb-4">
                        <div class="flex items-center gap-1.5">
                            ...
                            <span class="text-sm font-bold text-gray-800">{{ number_format($stats['total_views']) }}</span>
                        </div>
                        <div class="flex items-center gap-1.5">
                            <div class="w-2 h-2 rounded-full bg-teal-400"></div>
                            <span class="text-xs text-gray-500">Clicks</span>
                            <span class="text-sm font-bold text-gray-800">{{ number_format($stats['product_clicks']) }}</span>
                        </div>
                    </div>
                    <!-- Bar Chart -->
                        ...
                        @forelse($chartData as $day)
                            <div class="flex-1 flex flex-col items-center gap-1 h-full justify-end">
                                <div class="w-full flex flex-col items-center gap-0.5">
                                    @if(($day['views'] ?? 0) > 0)
                                        <div class="w-full bg-yellow-400 rounded-sm" style="height: {{ max(4, intval($day['views']) / max(1, $maxVal) * 80) }}px; min-height: 4px;"></div>
                                    @endif
                                    @if(($day['clicks'] ?? 0) > 0)
                                        <div class="w-full bg-teal-400 rounded-sm" style="height: {{ max(4, intval($day['clicks']) / max(1, $maxVal) * 80) }}px; min-height: 4px;"></div>
                                    @endif
                                </div>
                                <span class="text-[10px] text-gray-400">{{ \Carbon\Carbon::parse($day['date'])->format('d M') }}</span>
                            </div>
                        @empty
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
                ...
                <div class="p-5">
                    <a href="{{ route('seller.store.edit') }}#payout" class="flex items-center justify-between text-sm text-gray-600 hover:text-gray-900 transition">
...
<div class="flex items-center gap-2 mb-2">
            ...
            <p class="text-2xl font-bold text-gray-900">{{ number_format($stats['total_earnings'] ?? 0) }}</p>
        </div>
