...
<div class="py-6">
    ...
    <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-gray-500">Kelola informasi dan pengaturan etalase {{ $store->name }}</p>
    </div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-4 mb-6 flex items-center justify-between flex-wrap gap-3">
        <div class="flex items-center gap-2">
            ...
            <span class="text-sm font-semibold text-gray-800">{{ $store->public_url }}</span>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ $store->public_url }}" target="_blank"
 ...
 class="w-full rounded-xl border border-gray-300 px-4 py-2.5 text-sm focus:border-indigo-500 focus:ring-indigo-500 focus:outline-none transition"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="description" name="description" rows="3"
                                  class="w-full rounded-xl border border-gray-300 px-4 py-2.5 text-sm focus:border-indigo-500 focus:ring-indigo-500 focus:outline-none transition">{{ old('description', $store->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="url" id="logo" name="logo" value="{{ old('logo', $store->logo) }}"
                               placeholder="https://"
                               class="w-full rounded-xl border border-gray-300 px-4 py-2.5 text-sm focus:border-indigo-500 focus:ring-indigo-500 focus:outline-none transition">
                        @error('logo')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
    ...
    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden mb-6">
        ...
        <div class="p-6 flex items-center justify-between flex-wrap gap-4">
            <div>
                <p class="text-sm text-gray-600">Etalase {{ $store->is_active ? 'sedang aktif' : 'sedang nonaktif' }}</p>
            ...
            <form action="{{ route('seller.store.toggle-active') }}" method="POST">
                @csrf
                <button type="submit"
                        class="inline-flex items-center gap-2 px-4 py-2 {{ $store->is_active ? 'bg-red-50 text-red-600 border border-red-200 hover:bg-red-100' : 'bg-emerald-50 text-emerald-600 border border-emerald-200 hover:bg-emerald-100' }} rounded-xl font-semibold text-sm transition">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
