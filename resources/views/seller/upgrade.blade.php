...
@section('title', 'Upgrade ke Pro - EtalaseKu')
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-purple-700">
                            @if($store->plan_expires_at)
                                Berlangganan aktif hingga {{ $store->plan_expires_at->format('d MMMM Y') }}
                            @else
...
<li class="flex items-start gap-3">
        ...
        @unless($store->isPro())
            <div class="grid md:grid-cols-2 gap-6 mb-8">
                <!-- Monthly Plan -->
                <form method="POST" action="{{ route('seller.upgrade.checkout') }}">
                    @csrf
