        ...
        <aside class="fixed inset-y-0 left-0 z-30 flex h-full w-64 transform flex-col bg-gray-900 text-white transition-transform duration-200 lg:translate-x-0"
            ...
            <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
                ...
                <a href="{{ route('admin.verifications.index') }}"
                   class="flex items-center gap-3 px-4 py-3 rounded-lg transition {{ request()->routeIs('admin.verifications.*') ? 'bg-indigo-600 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @if($pendingVerifications > 0)
                        <span class="ml-auto bg-blue-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{{ $pendingVerifications }}</span>
                    @endif
                </a>
